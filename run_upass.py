...
def create_run_success_popup():
    '''
    Creates a new Tkinter popup window displaying text informing of a successful run completion of UPASS
    '''
    # Create tkinter window
    window = tk.Tk()

    # Set window title
    window.title('UPASS Run Complete')
    ...
    # Fill with labels
    label1 = Label(window, text="UPASS Run Successfully Complete", font=("Helvetica", 18))
    label1.pack(ipady=40, anchor='center')
...
def create_new_serp_directory(assignment_title):
    ...
    if not path.exists(path.join('bin/serp-results/' + assignment_title)):
        ...
        create_error_popup('New serp results directory could not be created for ' + assignment_title)
...
def get_urls_serpstack(search_term):
    ...
    if api_result.status_code != 200:
       create_error_popup('Invalid API response for ' + search_term)
...
def send_upload_confirmation_notification_email(email_addresses, assignment_title, course_contact_name, unique_serp_cheating_urls, end_date):
    ...
    try:
        ...
        create_error_popup('Failed to send new upload notification email for ' + assignment_title)
...
def main():
    ...
    # Notify user of successful run of UPASS check with popup
    create_run_success_popup()
